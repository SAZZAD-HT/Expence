...
const createSchema = z.object({
  ...
  budget_encrypted: z.string().min(1),
...
const patchSchema = z.object({
  ...
  budget_encrypted: z.string().min(1).optional(),
  notes_encrypted: z.string().optional(),
  status: z.enum(["planned", "bought", "cancelled"]).optional(),
});
...
function decryptRow(r: Record<string, unknown>) {
  return {
    ...
    budget_encrypted: decryptWithMaster(r.budget_encrypted as string),
...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await supabase
    ...
    .select("id, name_encrypted, budget_encrypted, status, notes_encrypted, created_at")
    .eq("user_id", user.id)
...
export async function POST(req: NextRequest) {
  ...
  const { name_encrypted, budget_encrypted, notes_encrypted } = parsed.data;
  const supabase = createServerClient();
  const { data, error } = await supabase
    .from("procurement_items")
    .insert({
      ...
      budget_encrypted: encryptWithMaster(budget_encrypted),
...
export async function PATCH(req: NextRequest) {
  ...
  const { id, name_encrypted, budget_encrypted, notes_encrypted, status } = parsed.data;
  const updates: Record<string, unknown> = {};
  if (name_encrypted) updates.name_encrypted = encryptWithMaster(name_encrypted);
  if (budget_encrypted) updates.budget_encrypted = encryptWithMaster(budget_encrypted);
